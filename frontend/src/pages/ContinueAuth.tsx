...
const useAuthenticateUser = (vfp: string, userAuthGuid: string | undefined, isMobile: boolean | undefined, env: string) => {
    ...
    useEffect(() => {
        if (vfp && userAuthGuid && env) {
            (async () => {
                try {
                    const result = await getInstantAuthResult(vfp, userAuthGuid, isMobile) as InstantAuthResult;
...
const ContinueAuth = ({ env, vfp, isMobile, isRedirected, onAuthSuccessMobile }: Props) => {
    const { t } = useTranslation();
    const { userAuthGuid } = useParams();
    const navigate = useNavigate();

    const { authResult, loading, verified } = useAuthenticateUser(vfp, userAuthGuid, isMobile, env);
