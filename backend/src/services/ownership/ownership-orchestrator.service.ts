...
import { PrefillResultsExtended, SuccessIdentityResponse, SuccessIdentityConfirmationResponse, IdentityServiceResponse } from '@src/services/ownership/(definitions)';
import { ADDRESS_SCORE_THRESHOLD, NAME_SCORE_THRESHOLD } from '@src/services/ownership/(constants)';
...
export default class OwnershipOrchestratorService {
  ...
  public async finalize(piiData:
    {
      ...
      region: string,
      postal_code: string,
    }): Promise<IdentityServiceResponse> {
    try {
      await this.getPrefillResult();
  ...
  private identityConfirmationCriteria(): boolean {
    ...
    const firstNameScore = (identityConfirmationResponse?.name?.firstName ?? 0) >= NAME_SCORE_THRESHOLD;
    const lastNameScore = (identityConfirmationResponse?.name?.lastName ?? 0) >= NAME_SCORE_THRESHOLD;
    const addressScore = (identityConfirmationResponse?.address?.addressScore ?? 0) >= ADDRESS_SCORE_THRESHOLD;
